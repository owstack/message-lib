...
describe('Message', function() {
  ...
  it('will error with incorrect message type', function() {
    expect(function() {
      return new Message({coinLib: btcLib}, new Date());
    }).to.throw('Second argument should be a string');
  });

  it('will instantiate without "new"', function() {
    var message = Message({coinLib: btcLib}, text);
  ...
  it('sign will error with incorrect private key argument', function() {
    expect(function() {
      var message3 = new Message({coinLib: btcLib}, text);
  ...
  it('verify will error with incorrect public key argument', function() {
    expect(function() {
      var message6 = new Message({coinLib: btcLib}, text);
  ...
  it('verify will error with incorrect signature argument', function() {
    expect(function() {
      var message7 = new Message({coinLib: btcLib}, text);
  ...
  it('verify will correctly identify a bad signature', function() {
    var message8 = new Message({coinLib: btcLib}, text);
  ...
  it('can verify a message with address and generated signature string', function() {
    var message9 = new Message({coinLib: btcLib}, text);
  ...
  it('will not verify with address mismatch', function() {
    var message10 = new Message({coinLib: btcLib}, text);
  ...
  it('will verify with an uncompressed pubkey', function() {
    ...
    var message = new Message({coinLib: btcLib}, 'This is an example of a signed message.');
  ...
  it('can chain methods', function() {
    var verified = Message({coinLib: btcLib}, text).verify(address, signatureString);
    verified.should.equal(true);
  });

  it('accepts BTC Address for verification', function() {
    var verified = Message({coinLib: btcLib}, text)
  ...
  describe('#json', function() {
    ...
    it('roundtrip to-from-to', function() {
      var json = new Message({coinLib: btcLib}, text).toJSON();
      var message = Message.fromJSON({coinLib: btcLib}, json);
      message.toString().should.equal(text);
    });

    it('checks that the string parameter is valid JSON', function() {
      expect(function() {
        return Message.fromJSON({coinLib: btcLib}, '¹');
      }).to.throw();
    });
  ...
  describe('#toString', function() {
    ...
    it('message string', function() {
      var message = new Message({coinLib: btcLib}, text);
      message.toString().should.equal(text);
    });

    it('roundtrip to-from-to', function() {
      var str = new Message({coinLib: btcLib}, text).toString();
      var message = Message.fromString({coinLib: btcLib}, str);
      message.toString().should.equal(text);
    });
