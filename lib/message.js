 ...
 * @returns {Message}
 */
function Message(message, networkName) {
  if (!(this instanceof Message)) {
    return new Message(message, networkName);
  }
  $.checkArgument(lodash.isString(message), 'First argument should be a string');
  $.checkArgument(lodash.isString(networkName), 'Second argument should be a string');

  this.message = message;
  this.coinLib = Networks.get(networkName).lib;
...
Message.prototype.verify = function verify(address, signatureString) {
  ...
  if (lodash.isString(address)) {
    address = this.coinLib.Address.fromString(address);
  }
  ...
  var signatureAddress = this.coinLib.Address.fromPublicKey(publicKey, address.network);
 ...
 */
Message.fromString = function(str, networkName) {
  return new Message(str, networkName);
};
 ...
 * @returns {Message} A new instance of a Message
 */
Message.fromJSON = function fromJSON(json, networkName) {
  if (JSUtil.isValidJSON(json)) {
    json = JSON.parse(json);
  }
  return new Message(json.message, networkName);
};
